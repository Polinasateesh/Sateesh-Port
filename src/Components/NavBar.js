...
const CustomMobileLink = ({ href, title, className = '', toggle }) => {
  ...
  return (
    ...
    >
      {title}
      <span
        className={`h-[1px] inline-block w-0 bg-light dark:bg-dark absolute left-0 -bottom-0.5
        group-hover:w-full transition-[width] ease duration-300 ${
          router.asPath === href ? 'w-full' : 'w-0'
        } `}
      >
...
const NavBar = () => {
  ...
  return (
    <header className="w-full px-32 py-8 font-medium flex items-center justify-between dark:text-light z-10 lg:px-16 md:px-12 sm:px-8 relative">
      <button
      ...
      {isOpen ? (
        <motion.div
          ...
          className="min-w-[70vw] flex flex-col justify-center items-center z-30 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 fixed bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-md py-32"
        >
